...
import { NeonButton } from "@/components/ui/neon-button";
import { StatGauge } from "@/components/ui/stat-gauge";
import { LogOut, Power, Activity, User, Cpu, Zap, Terminal, Thermometer, Fan } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { cn } from "@/lib/utils";
import { logout, getCurrentUser } from "@/lib/auth";
import { mqttClient } from "@/lib/mqttClient";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const handler = (topic: string, msg: Buffer) => {
      ...
      if (topic === "agv/raenaldiAS/vpin/V3") setBattery(+v);
      if (topic === "agv/raenaldiAS/vpin/V4") setAgvStatus(v as "STOPPED" | "RUNNING");
      if (topic === "agv/raenaldiAS/vpin/V6") setTemperature(+v);
      if (topic === "agv/raenaldiAS/vpin/V7") setFanStatus(v === "1" || v === "ON" ? "ON" : "OFF");
      if (topic === "agv/raenaldiAS/vpin/V8") setIsCharging(v === "1" || v === "TRUE" || v === "true");
